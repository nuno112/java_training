...
//get the teams info from json file
$.getJSON("storage/teams.json", function(data) {
	teams_ = data.teams;
});
...
if(previousPage == "login.html") {
	$(document).ready(function() {
		...
		if (sessionID == "null") {
			console.log("wtf happened?!? sessionID is null!!");
			alert("wtf happened?!? sessionID is null!!");
			window.location = "login.html";
		} else {
			$("#reset").click(function() {
				clear();
				$.getJSON("storage/tournamentdata.json", function(data) {
					populate(data, "reset");
					$(".sortable").sortable({
						helper:'clone',
			...
			$("#save").click(function() {
				...
				if (saveData != null && dataShown) {
					...
					$.ajax({
						url: "storage/tournamentdata.json",
						datatype: "json",
						async: false,
						success: function (data) {
							localStorage.setItem("user" + sessionID, data);
						}
					});
    ...
    return `
        <div class="group" id="${group.groupID}">
			...
			<div class="sortable">
				<div class="place">
					<span>1</span>
					<span title="team${teams_[group.team1 - 1].id}">
						${teams_[group.team1 - 1].name}
					</span>	
				</div>
				<div class="place">
					<span>2</span>
					<span title="team${teams_[group.team2 - 1].id}">
						${teams_[group.team2 - 1].name}
					</span>
				</div>
				<div class="place">
					<span>3</span>
					<span title="team${teams_[group.team3 - 1].id}">
						${teams_[group.team3 - 1].name}
					</span>
				</div>
				<div class="place">
					<span>4</span>
					<span title="team${teams_[group.team4 - 1].id}">
						${teams_[group.team4 - 1].name}
					</span>
				</div>
			</div>
		</div>
	...
	return `
		<div>
			...
			<div class="match" id="${match}">
				<div class="team1">
					<span title= "team${teams_[matchTeams.team1 - 1].id}" class="draggable">${teams_[matchTeams.team1 - 1].name}</span>
				</div>
				<div class="team2">
					<span title= "team${teams_[matchTeams.team2 - 1].id}" class="draggable">${teams_[matchTeams.team2 - 1].name}</span>
				</div>
			</div>
		</div>
		`;
...
function populate(dataObj, mode) {
	...
	$("#first span:nth-child(2)").text(teams_[dataObj.podium.first - 1].name);
	$("#first span:nth-child(2)").attr("title", "team" + teams_[dataObj.podium.first - 1].id);
	$("#second span:nth-child(2)").text(teams_[dataObj.podium.second - 1].name);
	$("#second span:nth-child(2)").attr("title", "team" + teams_[dataObj.podium.second - 1].id);
	$("#third span:nth-child(2)").text(teams_[dataObj.podium.third - 1].name);
	$("#third span:nth-child(2)").attr("title", "team" + teams_[dataObj.podium.third - 1].id);
...
function setDroppables() {
	...
	$("#third").droppable( {
		helper: "clone",
		accept: function(d) {
			console.log(d.parents().find("#match62")[0])
